...
public class MapEditorView extends UIView {
	...
	private PathTile mySelectedTile; // this is the tile that will be added on
										// mouse click
	...
	public MapEditorView(int xDim, int yDim, Frame frame) {
		...
		mySelectedTile = new PathTile(new TileData(defaultWalkImage, new Tuple<Integer, Integer>(0, 0), TileType.WALK),
		...
		for (int i = 0; i < myDimensions.x; i++) {
			for (int j = 0; j < myDimensions.y; j++) {
				...
				if (j == myDimensions.y / 2) // dirt
					tile = new PathTile(new TileData(defaultWalkImage, index, TileType.WALK), myTileWidth,
							myTileHeight);
				else // grass
					tile = new PathTile(new TileData(defaultBlockImage, index, TileType.BLOCK), myTileWidth,
							myTileHeight);
				setTile(tile);
			}
		}
		//TODO:make these clonable with right clicks (change to replace)
		setTile(new PathTile(
				new TileData(defaultEntryImage, new Tuple<Integer, Integer>(0, myDimensions.y / 2), TileType.ENTRY),
				myTileWidth, myTileHeight));
		setTile(new PathTile(new TileData(defaultExitImage,
				new Tuple<Integer, Integer>(myDimensions.x - 1, myDimensions.y / 2), TileType.EXIT), myTileWidth,
				myTileHeight));

	}

	private void setTile(PathTile tile) {
		myMapData.addTileDataAtIndex(tile.getTileData(), tile.getTileData().getIndex().x,
				tile.getTileData().getIndex().y);
	...
	private void replaceTile(PathTile currentTile, PathTile newTile) {
		myGridPane.getChildren().remove(currentTile);
		setTile(newTile);
	}
	...
	private void setupMouseEvents() {
		...
		this.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> onClick(e));
	}

	private void onClick(MouseEvent e) {
		Optional<PathTile> tile = Optional.ofNullable(findTileAtPoint(e.getX(), e.getY()));
		// PathTile tile = findTileAtPoint(e.getX(),e.getY());
		if (tile.isPresent()) {
			
			if (e.getButton().equals(MouseButton.SECONDARY)) {

				mySelectedTile = tile.get();
			}
			else {
				replaceTile(tile.get(), mySelectedTile.clone(tile.get().getTileData().getIndex()));
			}
		} 
	}
	...
	private void chopUpImage() {
		// TODO
	}

	private PathTile findTileAtPoint(double x, double y) {
		Optional<Node> node = myGridPane.getChildren().stream().filter(e -> e.getBoundsInParent().contains(x, y))
				.findFirst();
		if (node.isPresent()) {
